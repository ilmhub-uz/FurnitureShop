using FluentValidation;
using FurnitureShop.Admin.Api.Dtos;

namespace FurnitureShop.Admin.Api.Validators;

public class UpdateProductDtoValidator : AbstractValidator<UpdateProductDto>
{
    private readonly uint _minCountOfProduct = 0;
    public UpdateProductDtoValidator()
    {
        RuleFor(dto => dto.Name).NotNull();
        RuleFor(dto => dto.Price).NotNull();

        RuleFor(dto => dto.Count).NotNull();
        RuleFor(dto => dto.Count).GreaterThan(_minCountOfProduct);

        RuleFor(dto => dto.CategoryId).NotNull();
        RuleFor(dto => dto.CategoryId).GreaterThan(0);
        
        RuleFor(dto => dto.OrganizationId).NotEmpty();
    }
}